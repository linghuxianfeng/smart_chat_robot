...
static int smart_chat(char* send_text, char *recv_text, int recv_text_size, char *code, int recv_code_size)
{
	/*步骤1：创建socket*/
        ...
        //将IP地址转换为网络字节序
        inet_pton(AF_INET, "47.93.153.235", &serveraddr.sin_addr.s_addr);
        ...
        int params_len = strlen("key=ba91560ef9a34f7c8cb7f6e1a0487e05&info=") + strlen(send_text);
		...
		"key=%s&info=%s",
                params_len,
                "ba91560ef9a34f7c8cb7f6e1a0487e05",
                send_text
                );
	...
	else if (arecord_pid > 0)   //父进程执行
	{
		...
		{	
			...
			if (strlen(recv_text) < 1024)  
			{
				...
				//3、语音输出
				system("omxplayer -o local /home/pi/VoiceRecognitionControlRobot/wav/compose_voice_temp/compose_voice_all.wav");
				kill_omxplayer();
